...
public class BuildingController extends BaseController {
     ...
     */
    private void allProjectsFromConstruct(JSONObject resultJson, long userId) {
    ...
    public @ResponseBody ResponseEntity<String> getLicenseBuyInfo(@RequestParam("level") byte level, User user) {
        ...
        if (level < 1 || level > 4) {
            ...
            if (userSolvency < licensePrice) { // не хватает денег Util.moneyFormat(userSolvency) + "&tridot;</b>");
                ResponseUtil.putErrorMsg(resultJson,
                        "Не хватает денег на покупку. Ваш максимум: <b>" + CommonUtil.moneyFormat(userSolvency)
     ...
     * @return
     */
    public static Date checkLicenseExpire(Date licenseExpireDate, UserRep userRep, long userId) {
     ...
     * @param level
     */
    static Date setNewLicenseToUser(UserRep userRep, long userId, byte level) {
     ...
     */
    private long availableForBuildToday(long userId) {
